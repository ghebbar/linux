...
static int fb_probe(struct platform_device *device)
{
	...
	if (!IS_ERR(par->pinctrl)) {
		...
		if (IS_ERR(par->pins_default))
			dev_warn(&device->dev, "could not get default pinstate\n");
		else
		...
		if (IS_ERR(par->pins_sleep))
			dev_warn(&device->dev, "could not get sleep pinstate\n");
	} else {
		/*
		...
		*/
		...
		dev_warn(&device->dev, "did not get pins for i2c error: %li\n",
			 PTR_ERR(par->pinctrl));
	}
