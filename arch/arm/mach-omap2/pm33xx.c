...
static irqreturn_t wkup_m3_txev_handler(int irq, void *unused)
{
	...
	case M3_STATE_RESET:
		...
		wkup_m3->ver = omap_ctrl_readl(AM33XX_CONTROL_IPC_MSG_REG2) &
				M3_VERSION_MASK;
		if (wkup_m3->ver == M3_VERSION_UNKNOWN ||
			wkup_m3->ver < M3_BASELINE_VERSION) {
...
static void am33xx_pm_firmware_cb(const struct firmware *fw, void *context)
{
	...
	memcpy_toio(wkup_m3_context->code, fw->data, fw->size);
